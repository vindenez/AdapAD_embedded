...
class LSTMPredictor {
  public:
    ...
    void set_lstm_bias(int layer, const std::vector<float> &b_ih, const std::vector<float> &b_hh);
    ...
    std::vector<LSTMLayer> get_weights() const { return lstm_layers; }

    void set_weights(const std::vector<LSTMLayer> &weights);

    std::vector<LSTMGradients> get_last_gradients() const { return last_gradients; }
  ...
  protected:
    ...
    virtual void apply_sgd_update(std::vector<float> &biases, std::vector<float> &grads,
                                  float learning_rate, float momentum = 0.9f);
    ...
    std::vector<std::vector<std::vector<float>>>
        velocity_weight_ih; // [num_layers][4*hidden_size][input_size]
    std::vector<std::vector<std::vector<float>>>
        velocity_weight_hh;                             // [num_layers][4*hidden_size][hidden_size]
