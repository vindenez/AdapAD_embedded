...
protected:
    void SetUp() override {
        ...
        lstm = std::make_unique<LSTMPredictor>(
            num_classes, input_size, hidden_size, num_layers, seq_length
        );
    }

    // Helper function to create sample input
    std::vector<std::vector<std::vector<float>>> create_sample_input(
        size_t batch_size = 1, size_t seq_len = 5) {
        std::vector<std::vector<std::vector<float>>> input(
            batch_size, 
            std::vector<std::vector<float>>(
                seq_len,
                std::vector<float>(input_size, 0.1f)
            )
        );
        return input;
    }
    ...
    std::vector<LSTMPredictor::LSTMGradients> compute_numerical_gradients(
        ...
        DEBUG_PRINT(("Base loss: %f, Base prediction: %f, Target: %f\n", 
            base_loss, base_pred[0], target[0]));
        ...
        for (int layer = 0; layer < predictor->get_num_layers(); ++layer) {
            ...
            for (size_t i = 0; i < numerical_grads[layer].weight_ih_grad.size(); ++i) {
                for (size_t j = 0; j < numerical_grads[layer].weight_ih_grad[i].size(); ++j) {
                    ...
                    DEBUG_PRINT(("Computing gradient for weight_ih[%zu][%zu], original value: %f\n", 
                        i, j, original_weight));
            ...
            for (size_t i = 0; i < numerical_grads[layer].weight_hh_grad.size(); ++i) {
                for (size_t j = 0; j < numerical_grads[layer].weight_hh_grad[i].size(); ++j) {
                    ...
                    DEBUG_PRINT(("Computing gradient for weight_hh[%zu][%zu], original value: %f\n", 
                        i, j, original_weight));
            ...
            for (size_t i = 0; i < numerical_grads[layer].bias_ih_grad.size(); ++i) {
                ...
                numerical_grads[layer].bias_ih_grad[i] = 
                    (loss_plus - loss_minus) / (2 * epsilon);
            }
            ...
            for (size_t i = 0; i < numerical_grads[layer].bias_hh_grad.size(); ++i) {
                ...
                numerical_grads[layer].bias_hh_grad[i] = 
                    (loss_plus - loss_minus) / (2 * epsilon);
            }
        }
    ...
    float compare_gradients(
        const std::vector<LSTMPredictor::LSTMGradients>& analytical,
        ...
        for (size_t layer = 0; layer < analytical.size(); ++layer) {
            ...
            for (size_t i = 0; i < analytical[layer].bias_ih_grad.size(); ++i) {
                float diff = std::abs(
                    ...
                    numerical[layer].bias_ih_grad[i]);
                if (diff > 1e-3) {
            ...
            for (size_t i = 0; i < analytical[layer].bias_hh_grad.size(); ++i) {
                float diff = std::abs(
                    ...
                    numerical[layer].bias_hh_grad[i]);
                if (diff > 1e-3) {
...
TEST_F(LSTMPredictorTest, GradientCheck) {
    ...
    float epsilon = 1e-5;
    auto numerical_grads = compute_numerical_gradients(
        lstm.get(), input, target, epsilon);
